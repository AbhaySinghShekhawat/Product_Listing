  ...
  return (
    <div className="bg-white">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 sm:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-y-10 lg:gap-x-8">
          <div className="lg:col-span-1">
            ...
            <ul className="space-y-2 mt-4 hidden lg:block">
              <Link to={"/listing"}>
                <li className="shadow hover:bg-blue-400 hover:text-white cursor-pointer p-2 rounded-md">
                  All
                </li>
              </Link>
              {categories.map((cat, i) => (
                <Link to={`/listing/${cat.slug}`} key={i}>
                  <li
                    className={`${
                      category_slug === cat.slug ? "bg-blue-400 text-white" : ""
                    } shadow hover:bg-blue-400 hover:text-white cursor-pointer p-2 rounded-md`}
                  >
                    {cat.name}
                  </li>
                </Link>
            ...
            <div className="lg:hidden mt-4">
              ...
              {dropdownOpen && (
                <ul className="mt-2 space-y-2 bg-white shadow-lg rounded-md py-2">
                  <Link to={"/listing"}>
                    <li className="px-4 py-2 hover:bg-blue-400 hover:text-white cursor-pointer">
                      All
                    </li>
                  </Link>
                  {categories.map((cat, i) => (
              ...
              {loading
                ? [1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                    <div key={i} className="group relative animate-pulse">
                      <div className="aspect-square w-full rounded-md bg-gray-200 lg:h-80"></div>
                ...
                : products.map((product) => (
                    <div key={product.id}>
                      ...
                      <button
                        ...
                        className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 my-3 w-full dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"
                      >
                        Add to cart
                      </button>
                    </div>
